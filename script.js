...
async function loadMagazineData() {
    ...
    } catch (e) { console.error("Critical: Could not load data.json", e); }
}
...
function renderMagazines(mags) {
    ...
    grid.innerHTML = mags.map((mag) => `
        <div class="magazine-card" onclick="openMagazine(${mag.id})">
            <img src="${mag.coverImage}" alt="Magazine">
        </div>
    `).join('');
}
...
function openMagazine(id) {
    ...
    // Hard Wipe: Prevents double-book ghosting
    ...
    try {
        pageFlip = new St.PageFlip(container, {
            ...
            usePortrait: true,
            drawShadow: true, 
            maxShadowOpacity: 0.15, // Light shadow stops "black page" bug
        ...
        // Mapping buttons to the fresh instance
